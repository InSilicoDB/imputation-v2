...
```
docker build -t geneplaza/imputation2 .
```
#### step 0

```
docker run --rm -v $PWD/required_tools:/app/required_tools -v $PWD/0_check_vcf_build.slurm.sh:/app/0_check_vcf_build.slurm.sh -v $PWD:/input-data geneplaza/imputation2 /app/0_check_vcf_build.slurm.sh /input-data/<filename>.vcf <ouput-dir>
```
#### step 1

```
docker run --rm -v $PWD/required_tools:/app/required_tools -v $PWD/1_lift_vcfs_to_GRCh37.slurm.sh:/app/1_lift_vcfs_to_GRCh37.slurm.sh -v $PWD:/input-data geneplaza/imputation2 /app/1_lift_vcfs_to_GRCh37.slurm.sh /input-data/<filename>.vcf /input-data/<filename>.BuildChecked /input-data/lift/
```

#### step 2

```
docker run --rm -v $PWD/required_tools:/app/required_tools -v $PWD/2_Genotype_Harmonizer_QC1.slurm.sh:/app/2_Genotype_Harmonizer_QC1.slurm.sh -v $PWD:/input-data geneplaza/imputation2 /app/2_Genotype_Harmonizer_QC1.slurm.sh <in-dir> <ref-dir> <out-dir>
```
